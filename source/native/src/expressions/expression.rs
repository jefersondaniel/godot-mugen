    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let text = match self {
            ...
            Expression::FunctionCallExpression(name, expressions) => {
                let argument_names: Vec<String> = expressions
                    .iter()
            ...
            Expression::IntervalOperatorExpression(_, start_op, end_op, _, left, right) => {
                let start = if left.is_string() {
                    format!("\"{}\"", left.to_string())
                } else {
                    left.to_string()
                };
                let end = if right.is_string() {
                    format!("\"{}\"", right.to_string())
                } else {
                    right.to_string()
                };
                format!("{}{},{}{}", start_op, start, end, end_op)
            }
...
impl Expression {
    ...
    pub fn evaluate(&self, context: &Context) -> Value {
        match self {
            ...
            Expression::BinaryOperatorExpression(op, left, right) => match &op[..] {
                ...
                "!=" => left
                    .evaluate(context)
                    .equal(&right.evaluate(context))
                    .logical_not(),
                ">" => left.evaluate(context).greater(&right.evaluate(context)),
                ">=" => left
                    .evaluate(context)
                    .greater_or_equal(&right.evaluate(context)),
                "<" => left.evaluate(context).less(&right.evaluate(context)),
                "<=" => left
                    .evaluate(context)
                    .less_or_equal(&right.evaluate(context)),
                ...
                ":=" => {
                    ...
                    if let Expression::DynamicVariableExpression(base_name, argument) = left_deref {
                        let name =
                            &[base_name, &argument.evaluate(context).to_string()[..]].join(".");
                        context.assign(name, &assignment);
                        return assignment;
                    }
            ...
            Expression::UnaryOperatorExpression(op, right) => match &op[..] {
            ...
            Expression::IntervalOperatorExpression(
                op,
                start_op,
                end_op,
                expression,
            ...
            Expression::RedirectionExpression(left, right) => {
                ...
                } else if let Expression::DynamicVariableExpression(base_name, argument) =
                    left_deref
                {
                    context_name =
                        [base_name, &argument.evaluate(context).to_string()[..]].join(".");
