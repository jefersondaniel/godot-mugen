...
use crate::expressions::parser::{
    advance, advance_and_expects, advance_and_expects_message, expression, token, ParserState,
};
...
impl Token {
    ...
    pub fn nud(&self, state_ref: Rc<RefCell<ParserState>>) -> Rc<Expression> {
        match self {
            ...
            Token::IdentifierToken { name } => {
                ...
                {
                    advance_and_expects_message(
                        Rc::clone(&state_ref),
                        ",",
                        "Expected , after redirection keyword",
                    );
                ...
                if token(Rc::clone(&state_ref)).get_type_name() == "(" {
                    ...
                    let is_dynamic_variable: bool =
                        name == "var" || name == "fvar" || is_redirection_name(name);

                    if is_dynamic_variable {
                        if arguments.len() != 1 {
                            let mut state = state_ref.borrow_mut();
                            state.error =
                                format!("Expected exactly one argument at identifier: {}", name);
                            return Rc::new(Expression::BottomExpression);
                        }

                        let dynamic_variable = Rc::new(Expression::DynamicVariableExpression(
                            name.to_string(),
                        ...
                        if is_redirection_name(name) {
                            ...
                            let right = expression(Rc::clone(&state_ref), 5);
                            return Rc::new(Expression::RedirectionExpression(
                                dynamic_variable,
                                right,
                            ));
                        }
                    ...
                    return Rc::new(Expression::FunctionCallExpression(
                        name.to_string(),
                        arguments,
                    ));
                }
            ...
            Token::BinaryAndUnaryOperatorToken { op, .. } => Rc::new(
                Expression::UnaryOperatorExpression(op.to_string(), expression(state_ref, 100)),
            ),
            Token::UnaryOperatorToken { op, .. } => Rc::new(Expression::UnaryOperatorExpression(
                op.to_string(),
                expression(state_ref, 100),
            )),
            ...
            Token::CommaToken => {
                let mut state = state_ref.borrow_mut();
                state.error = "Unexpected comma".to_string();
                Rc::new(Expression::BottomExpression)
            }
    ...
    pub fn led(&self, state_ref: Rc<RefCell<ParserState>>, left: Rc<Expression>) -> Rc<Expression> {
        match self {
            Token::BinaryAndUnaryOperatorToken {
                op,
                right_associative,
                precedence,
                ..
            ...
            Token::ParenthesisOpenToken => {
                let mut state = state_ref.borrow_mut();
                state.error = "Unexpected (".to_string();
                Rc::new(Expression::BottomExpression)
            }
            Token::ReservedToken { name } => {
                let mut state = state_ref.borrow_mut();
            ...
            Token::CommaToken => {
                let mut expressions: Vec<Rc<Expression>> = vec![left];
